...
import { For, Show, Accessor, createSignal, Switch, Match } from 'solid-js';
import type { YoutubeVideoType } from '../service/youtube.service';
import { PODCAST_PLAYLIST_ID } from '@env';
...
const Podcasts: Component<PodcastsProps> = (props) => {
  const [playVideoId, setPlayVideoId] = createSignal<string | null>(null);

  const openMoreItems = () => {
    window.open(`https://www.youtube.com/playlist?list=${PODCAST_PLAYLIST_ID}`, '_blank');
  };
    ...
    <>
      <div class="widget_webinars">
          ...
          <Switch
            fallback={
              <div class="button-wrapper" onClick={openMoreItems}>
                <div class="button button--transparent-blue">{text.podcasts.action}</div>
              </div>
            }
          >
        ...
        <Show when={!!playVideoId()} keyed>
          <div class="video-container" onClick={openMoreItems}>
